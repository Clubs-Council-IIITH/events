...
def createHoliday(details: InputHolidayDetails, info: Info) -> HolidayType:
    ...
    return HolidayType.from_pydantic(created_holiday)

@strawberry.mutation
def editHoliday(id: str, details: InputHolidayDetails, info: Info) -> HolidayType:
    """
    ...
    holidaysdb.find_one_and_update(
        {"_id": id},
        {"$set": jsonable_encoder(details)}
    )
    updated_holiday = holidaysdb.find({"_id": id})

    return HolidayType.from_pydantic(updated_holiday)
